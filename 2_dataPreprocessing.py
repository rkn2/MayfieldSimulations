...
BALANCING_METHOD = 'SMOTE'  # Changed to SMOTE
...
### MODIFIED: Added logic to handle SMOTE error ###
if BALANCING_METHOD and sampler_class:
    print(f"  Balancing is ENABLED using {BALANCING_METHOD} (strategy='{SAMPLING_STRATEGY}')...")
    sampler = None
    try:
        # Check for SMOTE and handle small classes by adjusting k_neighbors
        if BALANCING_METHOD == 'SMOTE':
            min_class_size = y_train.value_counts().min()
            # SMOTE's k_neighbors must be less than the number of samples in the smallest class.
            if min_class_size <= 5:  # Default k_neighbors is 5
                k_neighbors_val = max(1, min_class_size - 1)  # Ensure k_neighbors is at least 1
                print(f"  Smallest class has {min_class_size} samples. Setting SMOTE k_neighbors to {k_neighbors_val}.")
                sampler = sampler_class(sampling_strategy=SAMPLING_STRATEGY, random_state=RANDOM_STATE,
                                        k_neighbors=k_neighbors_val)
            else:
                sampler = sampler_class(sampling_strategy=SAMPLING_STRATEGY, random_state=RANDOM_STATE)
        else:  # For RandomOverSampler or other methods
            sampler = sampler_class(sampling_strategy=SAMPLING_STRATEGY, random_state=RANDOM_STATE)

        print(f"  Instantiated {sampler.__class__.__name__} sampler.")
